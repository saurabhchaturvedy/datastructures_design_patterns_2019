...
public class LargestSubStringWithoutRepeatingChar {
	...
	public static void main(String[] args) {
		...
		String input = "ABCDKMLARTES";
		System.out.println(findLargestSubStringWithoutRepeatChars(input));
	}

	/*
	 * Algorithm: for each character in the string, find the subString till repeated
	 * characters and find the largest substring Repeat the above step for the all
	 * characters
	 */
	public static String findLargestSubStringWithoutRepeatChars(String input) {
		...
		for (int i = 0; i < input.length();) {
			...
			if (set.contains(c)) { // iterate till same character found in the set and if found we need to take the
									// sub string (i , current position)
			...
			i++;
			if (longestTillNow.indexOf(c) < 0) { // if given string contains only duplicate characters then it should
													// not added to String
				longestTillNow += c;
			}
			set.add(c);
